...
Celery.prototype.fetchOrders = function(order_status, payment_status, fulfillment_status, since) {
  return Q.promise(function(resolve, reject) {
    ...
    Celery.prototype.celery.request('orders' + (since ? since : ''), function(err, celeryOrders) {
      if (err) { reject(err); }
      var retItems = [];
...
Celery.prototype.preOrders = function() {
  return Q.promise(function(resolve, reject) {
    if (!Celery.prototype.initialized) { return reject(Celery.prototype.uninitializedError); }
    Celery.prototype.celery.request('orders/count?line_items.sku=TM-T2-02', function(err, orderCount) {
  ...
  return Q.promise(function(resolve, reject) {
    ...
    Celery.prototype.celery.request({url: 'orders/' + whiplashOrder.id + '/payment_charge', method:"POST"}, function(err, body) {
      if (err) { return reject(err); }
        ...
        zendesk.createUser(whiplashOrder.shipping_name, whiplashOrder.email, function(err,res) {
          if (err) {
            console.log(timestamp(),'Unable to create user',whiplashOrder.email);
            res = { id: zendesk.whiplashServerID };
          }
          zendesk.createTicket(
            res.id,
            whiplashOrder.originator_id+' failed to charge',
            body.meta.error.message,
            ['failed-to-charge'],
            function(err,id) {
              if (err) { console.log(timestamp(), 'Unable to create ticket on zendesk', err); }
              else { console.log(timestamp(), 'Created ticket number', id, 'because it failed to charge'); }
            }
          );
        });
